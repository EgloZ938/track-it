...
import MapView, { Marker, Callout, PROVIDER_GOOGLE } from 'react-native-maps'; // <<< CHANGEMENT ICI: MapView au lieu de ClusteredMapView
import { router } from 'expo-router';
import { Ionicons } from '@expo/vector-icons';
import * as Location from 'expo-location';
import { useFocusEffect } from '@react-navigation/native';

// Import des types et de apiService
import apiService from '../../services/apiService';
...
export default function MapScreen() {
  ...
  // <<< CHANGEMENT ICI: le type de mapRef est MapView
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      ...
      console.log("Valid tickets for map:", validTickets.length);
    } catch (error) {
  ...
  const getTypeProblemeInfo = (typeId: string) => {
    return typesProblemes.find(tp => tp.id === typeId) || { icone: 'alert-circle', couleur: '#64748B', libelle: 'Inconnu' };
  };
  ...
  return (
    <View style={styles.container}>
      <MapView // <<< CHANGEMENT ICI: MapView au lieu de ClusteredMapView
        ref={mapRef}
        ...
        // Propriétés spécifiques à ClusteredMapView (animateClusters, renderCluster) sont supprimées ici
      >
...
const styles = StyleSheet.create({
  ...
  // Les styles de cluster (clusterContainer, clusterText) ne sont plus utilisés ici car il n'y a pas de clustering
  // mais les laisser ne causera pas d'erreur.
  clusterContainer: {
    width: 40,
    height: 40,
    borderRadius: 20,
