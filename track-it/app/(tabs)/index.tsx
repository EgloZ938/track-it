...
import {
  TypeProbleme, TypeTransport, LigneTransport,
  LocationObjectCoords, LocationObject, Ticket
} from '../types';

// Import correct pour Expo Router
import { useNavigation, useLocalSearchParams } from 'expo-router'; // Utilisez useLocalSearchParams pour les paramètres de route
...
export default function EcranSignalement() {
  ...
  const params = useLocalSearchParams(); // Utilisez useLocalSearchParams pour récupérer les paramètres

  // Castez params.initialLocation si elle existe
  // Expo Router passe les objets complexes comme des chaînes JSON dans les params
  const initialLocation: LocationObjectCoords | undefined = params.initialLocation
    ? JSON.parse(params.initialLocation as string)
    : undefined;
  ...
  useEffect(() => {
    if (initialLocation) {
        ...
        Alert.alert("Localisation pré-remplie", "La position a été récupérée depuis la carte.");
    }
  }, [initialLocation]);
    ...
    try {
      const ticketData: Ticket = {
        ...
        location: localisation ? {
          latitude: localisation.coords.latitude,
          longitude: localisation.coords.longitude,
          accuracy: localisation.coords.accuracy || null,
        } : null,
      };

      const resultat = await apiService.post('/tickets', ticketData);
      console.log('Ticket envoyé avec succès:', resultat.data);
      ...
      console.error('Erreur lors de l\'envoi du signalement:', error.response?.data?.message || error.message);
      Alert.alert(
        'Erreur d\'envoi',
        error.response?.data?.message || 'Impossible d\'envoyer votre signalement. Veuillez réessayer.',
        [{ text: 'OK' }]
      );
    } finally {
      setEnChargement(false);
